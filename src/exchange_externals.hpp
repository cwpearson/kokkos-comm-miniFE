...
exchange_externals_functor {
	...
	void operator() (int i) const {
		d_view(_send_buffer)(i) = d_view(_x)(d_view(_elements_to_send)(i));
	}
                   ...
                   VectorType& x)
{
...
#ifndef GPU_MPI
  Scalar* x_external = h_view(x_coefs).data() + local_nrow;
#else
 ...
 /* for(size_t i=0; i<total_to_be_sent; ++i) {
  ...
  Scalar* s_buffer = h_view(send_buffer).data();
#else
  Scalar* s_buffer = d_view(send_buffer).data();
#endif
