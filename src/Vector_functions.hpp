         ...
         VectorType& w)
{
  ...
  if(alpha==1.0)
    Kokkos::V_Add(d_view(w.coefs),d_view(x.coefs),beta,d_view(y.coefs),size);
  else
    Kokkos::V_Add(d_view(w.coefs),alpha,d_view(x.coefs),beta,d_view(y.coefs),size);
      ...
      const Vector& y)
{
  ...
  magnitude result = Kokkos::V_Dot(d_view(x.coefs),d_view(y.coefs),n);
