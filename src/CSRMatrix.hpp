...
#include <Kokkos_DualViewHelper.hpp>
...
CSRMatrix {
  ...
  void init_kokkos_matrix() {
	  mat = Kokkos_Matrix("A",row_offsets.size()-1,num_cols,row_offsets(row_offsets.size()-1),d_view(packed_coefs),d_view(row_offsets),d_view(packed_cols));
  };
